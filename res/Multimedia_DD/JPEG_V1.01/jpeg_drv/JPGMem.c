...
void *Phy2VirAddr(UINT32 phy_addr, int mem_size)
{
	...
	if (reserved_mem == NULL) {
		LOG_MSG(LOG_ERROR, "Phy2VirAddr", "DD::Phyical to virtual memory mapping was failed!\r\n");
		return NULL;
	}
...
BOOL JPGMemMapping(S3C6400_JPG_CTX *base)
{
	...
	{
		LOG_MSG(LOG_ERROR, "JPGMemMapping", "DD::v_pJPG_REG: VirtualAlloc failed!\r\n");
		return FALSE;
	}
...
BOOL JPGBuffMapping(S3C6400_JPG_CTX *base)
{
	...
	{
		LOG_MSG(LOG_ERROR, "JPGBuffMapping", "DD::v_pJPGData_Buff: VirtualAlloc failed!\r\n");
		return FALSE;
	}
