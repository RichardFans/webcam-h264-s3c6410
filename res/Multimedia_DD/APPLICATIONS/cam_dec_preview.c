...
static void exit_from_app() 
{
	...
	switch(LCD_BPP)
	{
		case 16:
                        ...
                        fb_size_decode = g_LCD_Width * g_LCD_Height*2;
                         break;
		case 24:
                        //fb_size = LCD_WIDTH * LCD_HEIGHT * 4;
                        fb_size_preview = g_preview_Width * g_preview_Height* 4;
                        fb_size_decode = g_LCD_Width * g_LCD_Height*4;
	...
	close(in_fd);
        ...
        munmap(win0_fb_addr, fb_size_decode);
	close(dec_fb_fd);

        munmap(win1_fb_addr, fb_size_preview);
	close(pre_fb_fd);
}
...
{             
	...
	struct v4l2_format codec_fmt;
        ...
        g_LCD_Height =lcdsize[lcdnum][1];
         ...
         g_preview_Y = g_LCD_Height/2;


         g_preview_Width = g_LCD_Width/2;
         g_preview_Height = g_LCD_Height/2;

       //  g_preview_Width = (g_LCD_Width/2)/16*16;
         //g_preview_Height = (g_LCD_Height/2)/16*16;
         ...
         // g_codec_Width=g_LCD_Width/16*16;
         // g_codec_Height=g_LCD_Height/16*16;

       //   g_codec_Width=g_preview_Width;
         // g_codec_Height=g_preview_Height;

         char rgb_for_preview[LCD_WIDTH_MAX * LCD_HEIGHT_MAX * 4];	// MAX
         //char rgb_for_preview[preview_Width*preview_Height * 4];	// MAX
	...
	/* Window0 initialzation for previewing */
        ...
        if ((pre_fb_fd = fb_init(1, LCD_BPP, g_preview_X, g_preview_Y, g_preview_Width, g_preview_Height, &addr)) < 0)
                exit_from_app();
	...
	while (1) {
		...
		/* Write RGB frame to LCD frame buffer */
              //  draw(win1_fb_addr, &rgb_for_preview[0], LCD_WIDTH, LCD_HEIGHT, LCD_BPP);
                 draw(win1_fb_addr, &rgb_for_preview[0], g_preview_Width, g_preview_Height, LCD_BPP);
	...
	while (1) {
		...
		if (encoding_flag == TRUE) {
			...
			pthread_mutex_lock(&mutex);
                        ...
                        sprintf(&file_name[0], "Cam_encoding_%dx%d-%d.264", g_codec_Width, g_codec_Height,++film_cnt);
			...
			for(yuv_cnt=0; yuv_cnt < frame_num; yuv_cnt++){
				...
				if(frame_count == 1)
                                        encoded_buf = mfc_encoder_exe(enc_handle, g_yuv, g_YUV_Frame_Buffer_Size, 1, &encoded_size);
				else
                                        encoded_buf = mfc_encoder_exe(enc_handle, g_yuv, g_YUV_Frame_Buffer_Size, 0, &encoded_size);
...
void decoding_thread(void *arg)
{
	...
	pp_param.SrcCSpace		= YC420;
        pp_param.DstFullWidth	=  g_LCD_Width; //800;		// destination width
        pp_param.DstFullHeight	=  g_LCD_Height; //480;		// destination height
        pp_param.DstWidth		= g_LCD_Width; //800;
        pp_param.DstHeight		= g_LCD_Height; //480;
	...
	osd_info_to_driver.LeftTop_y	= 0;
        osd_info_to_driver.Width		=  g_LCD_Width; //800;	// display width
        osd_info_to_driver.Height		=  g_LCD_Height;//480;	// display height
