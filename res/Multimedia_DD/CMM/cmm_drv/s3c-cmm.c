...
static int s3c_cmm_open(struct inode *inode, struct file *file)
{
	...
	if(CodecMem == NULL){
		LOG_MSG(LOG_TRACE, "s3c_cmm_open", "CodecMem application failed\n");
		UnlockCMMMutex();
		return FALSE;
	}
		...
		int cmd, unsigned long arg)
{
	...
	switch (cmd) 
	{
		case IOCTL_CODEC_MEM_ALLOC:
			...
			ret = copy_from_user(&codec_mem_alloc_arg, (CODEC_MEM_ALLOC_ARG *)arg, sizeof(CODEC_MEM_ALLOC_ARG));
		...
		case IOCTL_CODEC_MEM_FREE:
			...
			ret = copy_from_user(&codec_mem_free_arg, (CODEC_MEM_FREE_ARG *)arg, sizeof(CODEC_MEM_FREE_ARG));
		...
		case IOCTL_CODEC_CACHE_FLUSH:
			...
			ret = copy_from_user(&codec_cache_flush_arg, (CODEC_CACHE_FLUSH_ARG *)arg, sizeof(CODEC_CACHE_FLUSH_ARG));
		...
		case IOCTL_CODEC_GET_PHY_ADDR:
			ret = copy_from_user(&codec_get_phy_addr_arg, (CODEC_GET_PHY_ADDR_ARG *)arg, sizeof(CODEC_GET_PHY_ADDR_ARG));
			...
			ret = copy_to_user((void *)arg, (void *)&codec_get_phy_addr_arg, sizeof(CODEC_GET_PHY_ADDR_ARG));
...
int s3c_cmm_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	LOG_MSG(LOG_TRACE, "s3c_cmm_mmap", "vma->vm_end - vma->vm_start = %d\n", offset);
	...
	if( remap_pfn_range(vma, vma->vm_start, pageFrameNo, size,	\
				vma->vm_page_prot) ) {
		LOG_MSG(LOG_ERROR, "s3c_cmm_mmap", "cmm remap error");
		return -EAGAIN;
	}
...
static void InsertNodeToAllocList(ALLOC_MEM_T *node, int inst_no)
{
	LOG_MSG(LOG_TRACE, "InsertNodeToAllocList", "[%d]instance (cached_p_addr : 0x%08x uncached_p_addr : 0x%08x size:%ld cacheflag : %d)\n",	\
	...
	AllocMemHead = node;
	LOG_MSG(LOG_TRACE, "InsertNodeToAllocList", "Finished InsertNodeToAllocList\n");
}
...
static void DeleteNodeFromAllocList(ALLOC_MEM_T *node, int inst_no)
{
	LOG_MSG(LOG_TRACE, "DeleteNodeFromAllocList", "[%d]instance (uncached_p_addr : 0x%08x cached_p_addr : 0x%08x size:%ld cacheflag : %d)\n",	\
		inst_no, node->uncached_p_addr, node->cached_p_addr, node->size, node->cacheFlag);
	
	if(node == AllocMemTail){
		LOG_MSG(LOG_TRACE, "DeleteNodeFromAllocList", "InValid node\n");
		return;
	}
...
static void DeleteNodeFromFreeList( FREE_MEM_T *node, int inst_no)
{
	LOG_MSG(LOG_TRACE, "DeleteNodeFromFreeList", "[%d]DeleteNodeFromFreeList(startAddr : 0x%08x size:%ld)\n", inst_no, node->startAddr, node->size);
	if(node == FreeMemTail){
		LOG_MSG(LOG_ERROR, "DeleteNodeFromFreeList", "InValid node\n");
		return;
	}
...
static ALLOC_MEM_T * GetCodecVirAddr(int inst_no, CODEC_MEM_ALLOC_ARG *in_param)
{
	...
	if(in_param->cacheFlag) {
		...
		if (p_allocMem->v_addr == NULL) {
			LOG_MSG(LOG_TRACE, "GetCodecVirAddr", "Mapping Failed [PA:0x%08x]\n\r", p_allocMem->cached_p_addr);
			return NULL;
		}
	}
	else {
...
static void PrintList()
{
	...
	for(node1 = AllocMemHead; node1 != AllocMemTail; node1 = node1->next){
		...
		LOG_MSG(LOG_TRACE, "PrintList", "[AllocList][%d] inst_no : %d p_addr : 0x%08x v_addr:0x%08x size:%ld cacheflag : %d\n", 
	...
	for(node2 = FreeMemHead; node2 != FreeMemTail; node2 = node2->next){
			LOG_MSG(LOG_TRACE, "PrintList", "[FreeList][%d] startAddr : 0x%08x size:%ld\n", count++, node2->startAddr , node2->size);
