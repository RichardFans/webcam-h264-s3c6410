...
{
	LOG_MSG(LOG_TRACE, "decodeBody", "decodeBody function\n");
...
void rewriteYUV(S3C6400_JPG_CTX *jCTX, UINT32 width, UINT32 orgwidth, UINT32 height, UINT32 orgheight)
{
	...
	LOG_MSG(LOG_TRACE, "rewriteYUV", "rewriteYUV function\n");
							...
							JPG_ENC_PROC_PARAM	*EncParam)
{
	...
	if(EncParam->width <= 0 || EncParam->width > MAX_JPG_WIDTH
		|| EncParam->height <=0 || EncParam->height > MAX_JPG_HEIGHT){
			LOG_MSG(LOG_ERROR, "encodeJPG", "DD::Encoder : Invalid width/height\r\n");
			return JPG_FAIL;
	}
	...
	LOG_MSG(LOG_TRACE, "encodeJPG", "EncParam->encType : %d\n", EncParam->encType);
	...
	if(ret != OK_ENC_OR_DEC){
		LOG_MSG(LOG_ERROR, "encodeJPG", "DD::JPG Encoding Error(%d)\n", ret);
		return JPG_FAIL;
	}
